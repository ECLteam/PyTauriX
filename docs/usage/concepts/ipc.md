...
- [pytaurix.ipc.Commands][]

PyTauriX implements IPC API consistent with rust tauri. Reading tauri's documentation is like reading PyTauriX's documentation.

### Commands

#### Registering Commands

You can register a command handler using the decorator [@Commands.command][pytaurix.ipc.Commands.command].
...
The currently supported signature pattern is [ArgumentsType][pytaurix.ipc.ArgumentsType]. You must ensure that the parameter names and type annotations are correct, and `@Commands.command` will check them.

```python
--8<-- "docs/snippets/concepts/ipc/reg_cmd.py"
```
...
However, in most cases, we want strong type checking when calling. Rust `tauri` achieves this through `serde`, while `pytaurix` uses [pydantic](https://github.com/pydantic/pydantic).

!!! info
    `pydantic` is a super-fast Python validation and serialization library written in `rust`/`pyo3` 🤓.

If you use [BaseModel][pydantic.BaseModel]/[RootModel][pydantic.RootModel] as the type annotation for the `body` parameter/return value, pytaurix will automatically serialize/deserialize it for you:

```python
--8<-- "docs/snippets/concepts/ipc/serde_body.py"
```
...
For types like `str`, it would be cumbersome to explicitly declare `#!python StrModel = RootModel[str]` every time. Since pytaurix `v0.7`, if you use types other than `bytes`/`BaseModel`/`RootModel` as the type annotation for the `body` parameter or return value, pytaurix will automatically convert them to [BaseModel][pydantic.BaseModel]/[TypeAdapter][pydantic.TypeAdapter] behind the scenes.

```python
--8<-- "docs/snippets/concepts/ipc/serde_body_as_any.py"
```
...
??? tip "Implementation details"
    ...
    ```python
    --8<-- "docs/snippets/concepts/ipc/serde_body_as_any_impl.py"
    ```
...
```python
--8<-- "docs/snippets/concepts/ipc/gen_handler.py"
```

The key point here is that **you must not close the `BlockingPortal` (i.e., do not exit the context manager) while [App.run][pytaurix.App.run] is still running**.
...
```python
--8<-- "docs/snippets/concepts/ipc/exit_stack.py"
```
...
```typescript
--8<-- "docs/snippets/concepts/ipc/calling_cmd.ts"
```
...
Similar to `FastAPI`, as long as you throw an [InvokeException][pytaurix.ipc.InvokeException] in the `command`, the promise will reject with the error message.

```python
--8<-- "docs/snippets/concepts/ipc/ret_exec.py"
```
...
```python
--8<-- "docs/snippets/concepts/ipc/accessing_headers.py"
```

```typescript
--8<-- "docs/snippets/concepts/ipc/accessing_headers.ts"
```
...
- [pytaurix.ipc.JavaScriptChannelId][] and [pytaurix.ipc.Channel][]
- [pytaurix.webview.WebviewWindow.eval][]
...
To use a `channel`, you only need to add the [JavaScriptChannelId][pytaurix.ipc.JavaScriptChannelId] field to the `BaseModel`/`RootModel`, and then use [JavaScriptChannelId.channel_on][pytaurix.ipc.JavaScriptChannelId.channel_on] to get a [Channel][pytaurix.ipc.Channel] instance.

!!! info
    `JavaScriptChannelId` itself is a `RootModel`, so you can directly use it as the `body` parameter.

```python
--8<-- "docs/snippets/concepts/ipc/py_channel.py"
```

```typescript
--8<-- "docs/snippets/concepts/ipc/js_channel.ts"
```

### Evaluating JavaScript

You can use [WebviewWindow.eval][pytaurix.webview.WebviewWindow.eval] to evaluate JavaScript code in the frontend.
...
- [pytaurix.Listener--examples][]
- [pytaurix.Emitter--examples][]
