...
requires-python = ">=3.9"
dependencies = ["pytaurix == 0.8.*"]  # (2)!

[project.entry-points.pytaurix]
...
2. This is the version at the time of writing this tutorial. There may be a newer version of pytaurix available when you use it.
...
--8<-- "docs/snippets/tutorial/using_pytaurix/__init__.py"
```

```python title="src-tauri/python/tauri_app/__main__.py"
--8<-- "docs/snippets/tutorial/using_pytaurix/__main__.py"
```

## Run pytaurix from rust
...
name = "tauri-app"
path = "src/main.rs"
required-features = ["pytaurix/standalone"]

[dependencies]
# ...
pytaurix = { version = "0.8" }  # (1)!
pyo3 = { version = "0.25" }  # (2)!
```
...
Also, enable the `pytaurix/standalone` feature:

```json title="src-tauri/tauri.conf.json"
{
    "build": {
        "features": ["pytaurix/standalone"]
    }
}
```
...
--8<-- "docs/snippets/tutorial/using_pytaurix/lib.rs"
```

```rust title="src-tauri/src/main.rs"
--8<-- "docs/snippets/tutorial/using_pytaurix/main.rs"
```
...
--8<-- "docs/snippets/tutorial/using_pytaurix/vite.config.ts"
```
