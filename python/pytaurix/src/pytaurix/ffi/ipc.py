...
from typing import TYPE_CHECKING, Annotated, Any, Generic, final
...
_ipc_mod = pytaurix_mod.ipc

if TYPE_CHECKING:
    from pytaurix.ffi.lib import AppHandle
    from pytaurix.ffi.webview import Webview, WebviewWindow

_InvokeResponseBody = TypeAliasType("_InvokeResponseBody", str | bytes)
...
class State:
    """A marker for state in [ArgumentsType][pytaurix.ffi.ipc.ArgumentsType].
...
class ParametersType(TypedDict, total=False, closed=True):
    ...
    You can use this with [Invoke.bind_to][pytaurix.ffi.ipc.Invoke.bind_to].
    All keys are optional. If a key exists, it will be assigned a value corresponding to
    [ResolvedArgumentsType][pytaurix.ffi.ipc.ResolvedArgumentsType].
    """
...
class ResolvedArgumentsType(_BaseArgumentsType, total=False):
    ...
    It contains the resolved values of the parameters in [ParametersType][pytaurix.ffi.ipc.ParametersType].
    """

    states: dict[str, Annotated[Any, State()]]
    """The value is an instance of the state class in [ParametersType.states][pytaurix.ffi.ipc.ParametersType.states]."""
...
class ArgumentsType(
    ...
    Each key is optional, depending on the keys of the bound [ResolvedArgumentsType][pytaurix.ffi.ipc.ResolvedArgumentsType].
...
if TYPE_CHECKING:
    ...
    class Invoke:
        ...
        def bind_to(
        ...
        ) -> InvokeResolver[_ArgumentsTypeVar] | None:
            ...
            The return value [InvokeResolver.arguments][pytaurix.ffi.ipc.InvokeResolver.arguments]
