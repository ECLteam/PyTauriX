...
class InputOutput(NamedTuple):
    cmd_handler: object
    input_type: _Model | _Bytes | _Void
    output_type: _Model | _Bytes | _Void
...
class _InputOutputSerde(NamedTuple):
    input_type: tuple[_Model, Literal["validation"]] | _Bytes | _Void
    output_type: tuple[_Model, Literal["serialization"]] | _Bytes | _Void
...
def _is_model(
    type_: _Model | _Bytes | _Void,
) -> TypeIs[_Model]:
    return issubclass(type_, BaseModel)


def _is_model_serde(
    type_: tuple[_Model, JsonSchemaMode] | _Bytes | _Void,
...
def _gen_client_code(
    cmd_in_out: CommandInputOutput, *, cmd_alias: _AliasGenerator | None = None
) -> str:
...
async def gen_ts(
    ...
    json2ts_cli: str,
    *,
    cmd_alias: _AliasGenerator | None = None,
):
...
def _gen_ts_doc(obj: object) -> str | None:
    py_doc = getdoc(obj)
