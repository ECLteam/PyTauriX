...
use pytaurix_core::{tauri_runtime::Runtime, utils::TauriError};
use tauri::{
    image::Image,
...
struct ContextFactoryKwargs {
    // TODO: use `pytaurix::ext_mod::ConfigFrom` (`tauri::Config`) as the type
...
pub fn context_factory(
    ...
    let result: FactoryResult<TauriContext> = py.detach(move || {
