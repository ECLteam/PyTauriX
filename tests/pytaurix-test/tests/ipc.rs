...
use pytaurix_test::test::{ext_mod, Runtime};

static PYI: LazyLock<PythonInterpreter> = LazyLock::new(|| {
    let virtual_env = var("VIRTUAL_ENV").expect(
        "pytaurix IPC tests require an active virtual environment; run them with `uv run --package pytaurix-test --no-dev cargo test -p pytaurix-test --features test --locked`",
    );
...
{
    PYI.with_gil(|py| {
        let test_mod = py.import("pytaurix_test")?;
        ...
        let py_app_handle = context_manager
            ...
            .cast_into::<pytaurix::ext_mod::AppHandle>()?;
...
pub fn get_pytaurix_ipc_response<D: DeserializeOwned>(
    ...
    let resp = get_ipc_response(
        webview,
        InvokeRequest {
            cmd: "plugin:pytaurix|pyfunc".into(),
...
fn test_ipc() -> Result<(), Box<dyn Error>> {
    app_handle_fixture(|app| {
        ...
        let resp: String = get_pytaurix_ipc_response(&webview, "command".into(), &body);
        assert_eq!(resp, "pong");
