//! This crate is currently only used internally by pytaurix to
...
use pyo3::prelude::*;
use pytaurix_core::tauri_runtime::Runtime as PyTauriXRuntime;
use tauri::plugin::{Builder, TauriPlugin};
use tauri::{Manager, Runtime};

use crate::commands::invoke_handler;

const PLUGIN_NAME: &str = "pytaurix";

type PyInvokeHandlerType = Py<PyAny>;
...
///
/// > def py_invoke_handler(invoke: [Invoke][pytaurix_core::ext_mod::ipc::Invoke], /) -> None:
/// >     ...
///
...
pub fn init(py_invoke_handler: PyInvokeHandlerType) -> TauriPlugin<PyTauriXRuntime> {
    Builder::<PyTauriXRuntime>::new(PLUGIN_NAME)
