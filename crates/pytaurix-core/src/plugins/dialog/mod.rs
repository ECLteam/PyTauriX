...
impl From<PluginError> for PyErr {
    fn from(value: PluginError) -> Self {
        match value.0 {
            ...
            plugin::Error::Fs(e) => pytaurix_plugins::fs::PluginError::from(e).into(),
...
//
// ```rust
// // TODO: unify/stable this in `pytaurix_core::ext_mod`
// #[non_exhaustive]
// #[derive(FromPyObject)]
...
impl MessageDialogBuilder {
    ...
    fn blocking_show(&self, py: Python<'_>, kwargs: Option<&Bound<'_, PyDict>>) -> PyResult<bool> {
        ...
        let ret = py.detach(|| builder.blocking_show());
        Ok(ret)
    }
        ...
        let ret = match &self.0 {
            plugin::FilePath::Url(_) => {
                ...
                return Err(PyNotImplementedError::new_err(
                    "[FilePath::Url] type is only used on Android/iOS, report this to the pytaurix developers"
                ));
            }
...
impl FileDialogBuilder {
    #[pyo3(signature = (handler, /, **kwargs))]
    fn pick_file(&self, handler: Py<PyAny>, kwargs: Option<&Bound<'_, PyDict>>) -> PyResult<()> {
    ...
    fn blocking_pick_file(
        ...
        let ret = py.detach(|| builder.blocking_pick_file().map(Into::into));
        Ok(ret)
    }

    #[pyo3(signature = (handler, / ,**kwargs))]
    fn pick_files(&self, handler: Py<PyAny>, kwargs: Option<&Bound<'_, PyDict>>) -> PyResult<()> {
        let args = FileDialogBuilderArgs::from_kwargs(kwargs)?;
    ...
    #[pyo3(signature = (handler, / ,**kwargs))]
    fn pick_folder(&self, handler: Py<PyAny>, kwargs: Option<&Bound<'_, PyDict>>) -> PyResult<()> {
        let args = FileDialogBuilderArgs::from_kwargs(kwargs)?;
    ...
    fn blocking_pick_folder(
        ...
        let ret = py.detach(|| builder.blocking_pick_folder().map(Into::into));
        Ok(ret)
    }

    #[pyo3(signature = (handler, / ,**kwargs))]
    fn pick_folders(&self, handler: Py<PyAny>, kwargs: Option<&Bound<'_, PyDict>>) -> PyResult<()> {
        let args = FileDialogBuilderArgs::from_kwargs(kwargs)?;
    ...
    #[pyo3(signature = (handler, / ,**kwargs))]
    fn save_file(&self, handler: Py<PyAny>, kwargs: Option<&Bound<'_, PyDict>>) -> PyResult<()> {
        let args = FileDialogBuilderArgs::from_kwargs(kwargs)?;
    ...
    fn blocking_save_file(
        ...
        let ret = py.detach(|| builder.blocking_save_file().map(Into::into));
        Ok(ret)
    }
}
