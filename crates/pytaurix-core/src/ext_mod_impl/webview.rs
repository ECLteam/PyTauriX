...
// see [<PySerde as FromPyObject>::extract] comment.
...
impl WebviewWindow {
    ...
    fn run_on_main_thread(&self, py: Python<'_>, handler: Py<PyAny>) -> PyResult<()> {
        py.detach(|| {
    ...
    fn on_window_event(&self, py: Python<'_>, handler: Py<PyAny>) {
        py.detach(|| {
            self.0.inner_ref().on_window_event(move |window_event| {
                Python::attach(|py| {
                    let window_event: WindowEvent = WindowEvent::from_tauri(py, window_event)
                        ...
                        .expect("Failed to convert `WindowEvent` to Py<PyAny>");
    ...
    fn on_webview_event(&self, py: Python<'_>, handler: Py<PyAny>) {
        py.detach(|| {
            self.0.inner_ref().on_webview_event(move |webview_event| {
                Python::attach(|py| {
                    let webview_event: WebviewEvent = WebviewEvent::from_tauri(py, webview_event)
                        // TODO: maybe we should only `write_unraisable` and log it instead of `panic` here?
                        .expect("Failed to convert `WebviewEvent` to Py<PyAny>");
    ...
    fn on_menu_event(slf: Py<Self>, py: Python<'_>, handler: Py<PyAny>) {
            ...
            slf.get()
                ...
                .on_menu_event(move |_window, menu_event| {
                    Python::attach(|py| {
                        ...
                        // so we can directly use the same Py<PyAny>.
    ...
    fn menu(&self, py: Python<'_>) -> Option<Menu> {
        py.detach(|| self.0.inner_ref().menu().map(Menu::new))
    }
    ...
    fn hide_menu(&self, py: Python<'_>) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, hide_menu,))
    }

    fn show_menu(&self, py: Python<'_>) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, show_menu,))
    }

    fn is_menu_visible(&self, py: Python<'_>) -> PyResult<bool> {
        py.detach(|| delegate_inner!(self, is_menu_visible,))
    }

    fn popup_menu(&self, py: Python<'_>, menu: ImplContextMenu) -> PyResult<()> {
        py.detach(|| context_menu_impl!(&menu, |menu| delegate_inner!(self, popup_menu, menu)))
    }
    ...
    fn theme(&self, py: Python<'_>) -> PyResult<Theme> {
        py.detach(|| delegate_inner!(self, theme,).map(Into::into))
    }

    fn cursor_position(&self, py: Python<'_>) -> PyResult<PhysicalPositionF64> {
        let position = py.detach(|| delegate_inner!(self, cursor_position,))?;
        PhysicalPositionF64::from_tauri(py, position)
    }

    fn center(&self, py: Python<'_>) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, center,))
    }
    ...
    fn request_user_attention(
        ...
        py.detach(|| delegate_inner!(self, request_user_attention, attention_type.map(Into::into)))
    }

    fn set_resizable(&self, py: Python<'_>, resizable: bool) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, set_resizable, resizable))
    }

    fn set_enabled(&self, py: Python<'_>, enabled: bool) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, set_enabled, enabled))
    }

    fn set_maximizable(&self, py: Python<'_>, maximizable: bool) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, set_maximizable, maximizable))
    }

    fn set_minimizable(&self, py: Python<'_>, minimizable: bool) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, set_minimizable, minimizable))
    }

    fn set_closable(&self, py: Python<'_>, closable: bool) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, set_closable, closable))
    }

    fn set_title(&self, py: Python<'_>, title: &str) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, set_title, title))
    }

    fn maximize(&self, py: Python<'_>) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, maximize,))
    }

    fn unmaximize(&self, py: Python<'_>) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, unmaximize,))
    }

    fn minimize(&self, py: Python<'_>) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, minimize,))
    }

    fn unminimize(&self, py: Python<'_>) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, unminimize,))
    }

    fn show(&self, py: Python<'_>) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, show,))
    }

    fn hide(&self, py: Python<'_>) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, hide,))
    }

    fn close(&self, py: Python<'_>) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, close,))
    }

    fn destroy(&self, py: Python<'_>) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, destroy,))
    }

    fn set_decorations(&self, py: Python<'_>, decorations: bool) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, set_decorations, decorations))
    }

    fn set_shadow(&self, py: Python<'_>, shadow: bool) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, set_shadow, shadow))
    }
    ...
    fn set_always_on_bottom(&self, py: Python<'_>, always_on_bottom: bool) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, set_always_on_bottom, always_on_bottom))
    }

    fn set_always_on_top(&self, py: Python<'_>, always_on_top: bool) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, set_always_on_top, always_on_top))
    }
    ...
    fn set_zoom(&self, py: Python<'_>, scale_factor: f64) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, set_zoom, scale_factor))
    }

    fn clear_all_browsing_data(&self, py: Python<'_>) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, clear_all_browsing_data,))
    }

    fn cookies_for_url(&self, py: Python<'_>, url: Url<'_>) -> PyResult<Vec<Cookie>> {
        let cookies = py.detach(|| delegate_inner!(self, cookies_for_url, url.into()))?;
        let cookies = cookies
            .into_iter()
    ...
    fn cookies(&self, py: Python<'_>) -> PyResult<Vec<Cookie>> {
        let cookies = py.detach(|| delegate_inner!(self, cookies,))?;
        let cookies = cookies
            .into_iter()
            .map(|c| Cookie::from_tauri(py, &c))
            .collect::<Vec<_>>();
    ...
    fn set_cookie(&self, py: Python<'_>, cookie: Cookie) -> PyResult<()> {
        ...
        py.detach(|| delegate_inner!(self, set_cookie, cookie))
    }

    fn delete_cookie(&self, py: Python<'_>, cookie: Cookie) -> PyResult<()> {
        let cookie = cookie.to_tauri(py)?;
        py.detach(|| delegate_inner!(self, delete_cookie, cookie))
    }
...
impl<'a, 'py> FromPyObject<'a, 'py> for Cookie {
    type Error = PyErr;
...
pub struct WebviewWindowBuilderArgs {
    // TODO, FIXME: on_menu_event: NotRequired<Py<PyAny>>,
    ...
    on_navigation: NotRequired<Py<PyAny>>,
    // TODO: on_new_window
    on_document_title_changed: NotRequired<Py<PyAny>>,
