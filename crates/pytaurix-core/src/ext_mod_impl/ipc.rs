    ...
    fn bind_to(&self, parameters: &Bound<'_, PyDict>) -> PyResult<Option<InvokeResolver>> {
            ...
            match message.payload() {
                InvokeBody::Json(_) => {
                    resolver.reject(
                        ...
                        If you are using `pyInvoke`, please report this as bug to pytaurix developers.",
                    );
                    return Ok(None);
                }
        ...
        if parameters.contains(headers_key)? {
            let headers: Vec<(&[u8], &[u8])> = message
                ...
                .collect();
            // TODO: Unify and export this type in [crate::ext_mod::ipc], see Python [pytaurix.ipc.Headers] type.
        ...
        if let Some(states_params) = parameters.get_item(states_key)? {
            let states_params = states_params.cast_into::<PyDict>()?;
            ...
            for (key, value) in states_params.into_iter() {
                let state_type = value.cast::<PyType>()?;
...
impl JavaScriptChannelId {
    ...
    /// so that user dont need create new `Webview` Py<PyAny> for `WebviewWindow`.
