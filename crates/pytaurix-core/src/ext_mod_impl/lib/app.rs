        ...
        move |_app_handle, run_event| {
            ...
            Python::attach(|py| {
                let py_run_event: RunEvent = RunEvent::from_tauri(py, run_event)
                    ...
                    .expect("Failed to convert rust `RunEvent` to Py<PyAny>");
...
impl App {
    ...
    fn run(&self, py: Python<'_>, callback: Option<Py<PyAny>>) -> PyResult<()> {
    ...
    #[pyo3(signature = (callback = None, /))]
    fn run_return(&self, py: Python<'_>, callback: Option<Py<PyAny>>) -> PyResult<i32> {
        let app = self.0.try_take_inner()??;
        let py_app_handle = app.py_app_handle().clone_ref(py);
        unsafe {
            // Safety: `tauri::App` does not hold the GIL, so this is safe
    ...
    #[expect(deprecated)]
    #[pyo3(signature = (callback = None, /))]
    fn run_iteration(&self, py: Python<'_>, callback: Option<Py<PyAny>>) -> PyResult<()> {
