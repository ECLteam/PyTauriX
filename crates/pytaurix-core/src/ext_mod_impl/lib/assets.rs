...
impl Iterator for PyAssetsIter {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let item: Option<Self::Item> = Python::attach(|py| {
            let mut slf = self
                .0
...
pub(crate) struct PyAssets(pub(crate) Py<PyAny>);
...
impl Assets<Runtime> for PyAssets {
    fn get(&self, key: &TauriAssetKey) -> Option<Cow<'_, [u8]>> {
        ...
        let result = Python::attach(|py| {
            ...
            let result = (|| {
                ...
                let ret_py_bytes = ret.cast_into::<PyBytes>()?.unbind();
    ...
    fn iter(&self) -> Box<AssetsIter<'_>> {
        ...
        let assets_iter = Python::attach(|py| {
            let slf = self.0.bind(py);
            let result = (|| {
