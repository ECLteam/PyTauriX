...
use pyo3::{
    exceptions::PyValueError, prelude::*, types::PyString, Borrowed, FromPyObject, IntoPyObject,
};
...
// Parsing always produces an owned `tauri::Url`; retaining the borrowed Python
// input lifetime here would make `Url<'static>` fields impossible to extract.
impl<'a, 'py> FromPyObject<'a, 'py> for Url<'static> {
    type Error = PyErr;

    fn extract(ob: Borrowed<'a, 'py, PyAny>) -> PyResult<Self> {
        let url: String = ob.extract()?;
