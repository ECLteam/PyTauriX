...
pub(crate) fn debug_assert_app_handle_py_is_rs(
    ...
    if cfg!(debug_assertions) {
        ...
        debug_assert!(
            ...
            "AppHandle Py<PyAny> instance is not the same as the rust instance"
        );
    }
}
...
impl AppHandle {
    ...
    fn remove_plugin(&self, py: Python<'_>, plugin: Cow<'_, str>) -> bool {
        py.detach(|| self.0.inner_ref().remove_plugin(&plugin))
    }

    fn exit(&self, py: Python<'_>, exit_code: i32) {
        py.detach(|| self.0.inner_ref().exit(exit_code))
    }

    /// NoReturn
    #[expect(unreachable_code, reason = "Tauri restart intentionally never returns")]
    fn restart(&self, py: Python<'_>) {
        let _: Infallible = py.detach(|| self.0.inner_ref().restart());
    }

    fn request_restart(&self, py: Python<'_>) {
        py.detach(|| self.0.inner_ref().request_restart())
    }
    ...
    fn set_dock_visibility(&self, py: Python<'_>, visible: bool) -> PyResult<()> {
        py.detach(|| delegate_inner!(self, set_dock_visibility, visible))
    }

    // TODO: set_device_event_filter, see <https://github.com/tauri-apps/tauri/pull/14008>

    pub(crate) fn on_menu_event(slf: Py<Self>, py: Python<'_>, handler: Py<PyAny>) {
    ...
    fn on_tray_icon_event(slf: Py<Self>, py: Python<'_>, handler: Py<PyAny>) {
        let moved_slf = slf.clone_ref(py);
        py.detach(|| {
            slf.get()
                .0
                    ...
                    Python::attach(|py| {
                        ...
                        let tray_icon_event: TrayIconEvent =
                            TrayIconEvent::from_tauri(py, &tray_icon_event)
                                ...
                                .expect("Failed to convert rust `TrayIconEvent` to Py<PyAny>");
    ...
    fn tray_by_id(&self, py: Python<'_>, id: &str) -> Option<TrayIcon> {
        py.detach(|| self.0.inner_ref().tray_by_id(id).map(TrayIcon::new))
    }

    fn remove_tray_by_id(&self, py: Python<'_>, id: &str) -> Option<TrayIcon> {
        py.detach(|| self.0.inner_ref().remove_tray_by_id(id).map(TrayIcon::new))
    }
    ...
    fn set_theme(&self, py: Python<'_>, theme: Option<Theme>) {
        py.detach(|| self.0.inner_ref().set_theme(theme.map(Into::into)))
    }
    ...
    fn cleanup_before_exit(&self, py: Python<'_>) {
        py.detach(|| self.0.inner_ref().cleanup_before_exit())
    }
...
/// i.e. it was not created by pytaurix.
#[derive(Debug)]
...
impl Display for PyAppHandleStateError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Failed to get `PyAppHandle` from state, maybe this app was not created by pytaurix"
        )
    }
}
