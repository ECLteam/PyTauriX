...
use pytaurix_core::{ext_mod::PyAppHandleExt as _, tauri_runtime::Runtime, utils::TauriError};

/// Use [pymodule_export] instead of this [ext_mod] and [pytaurix_plugins] directly.
pub use pytaurix_core::{ext_mod, pytaurix_plugins};
...
pub struct BuilderArgs {
    /// see [`tauri_plugin_pytaurix::init`] for `invoke_handler`.
    invoke_handler: Option<Py<PyAny>>,
    /// see [tauri::Builder::setup] and python side type hint.
    setup: NotRequired<Py<PyAny>>,
...
derive_from_py_dict!(BuilderArgs {
    ...
    // see: <https://github.com/ECLteam/PyTauriX/pull/133>.
    invoke_handler,
    #[pyo3(default)]
    setup,
    #[pyo3(default)]
...
impl BuilderArgs {
    ...
    fn apply_to_builder(self, py: Python<'_>, mut builder: TauriBuilder) -> PyResult<TauriBuilder> {
        ...
        if let Some(invoke_handler) = invoke_handler {
            builder = builder.plugin(tauri_plugin_pytaurix::init(invoke_handler.clone_ref(py)));
        }
...
/// Exports the [ext_mod] and [pytaurix_plugins] module to the `parent_module`.
///
/// `context_factory` and `builder_factory` will be exported as the
/// `pytaurix.context_factory` and `pytaurix.builder_factory` functions on the
/// Python side.
///
/// # Example
/**
...
pub fn pymodule_export(
    ...
    let pytaurix_module: Py<PyModule> = wrap_pymodule!(ext_mod)(py);
    let pytaurix_module = pytaurix_module.bind(py);

    pytaurix_module.add_function(builder_factory)?;
    pytaurix_module.add_function(context_factory)?;
    pytaurix_module.add_class::<Builder>()?;

    let pytaurix_plugins_module = wrap_pymodule!(pytaurix_plugins)(py);
    let pytaurix_plugins_module = pytaurix_plugins_module.bind(py);

    parent_module.add_submodule(pytaurix_module)?;
    parent_module.add_submodule(pytaurix_plugins_module)?;
    Ok(())
}
